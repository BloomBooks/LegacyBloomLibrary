...
//                {books:{"__op":"AddRelation","objects":[{"__type":"Pointer","className":"books","objectId":book.objectId}]}});
        ...
        this.showHarvestedPdf = function (book) {
            // show the uploaded pdf if book is not harvested with pdf marked suitable
            return this.showArtifact(book, "pdf") && book.show;
        };
            ...
            if (book.show[artifact].user === undefined) {
                if (book.show[artifact].librarian === undefined) {
                    if (book.show[artifact].harvester === undefined) {
                        // this shouldn't happen, but just in case, show the book by default
                        return true;
                    } else {
